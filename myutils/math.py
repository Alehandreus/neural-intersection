...
def sph2coord(theta, phi, r=1.0):
    coord = torch.stack(
        [
            torch.sin(theta) * torch.cos(phi) * r,
...
def get_coord_from_pts(wcoords, w2c, scene_info):
    ...
    x = (
        (K[0, 2] + (coords[..., :1] * K[0, 0]) / (coords[..., 2:3] * K[2, 2] + EPS))
        .long()
        .squeeze(-1)
    )
    y = (
        (K[1, 2] + (coords[..., 1:2] * K[1, 1]) / (coords[..., 2:3] * K[2, 2] + EPS))
        .long()
        .squeeze(-1)
    )
    return x, y
...
def convert_d(d, scene_info, out="dist"):
    ...
    L = np.sqrt(np.power(j - H / 2.0, 2) + np.power(i - W / 2.0, 2) + focal**2)
    fl = focal / L
    if out == "dist":
        return d / fl
    elif out == "dep":
...
def get_surface_normal(t, raydirs):
    ...
    dtheta = torch.cat(
        [
            (dtdtheta * sin_theta + t * cos_theta) * cos_phi,
    ...
    dphi = torch.cat(
        [
            (dtdphi * cos_phi - t * sin_phi) * sin_theta,
